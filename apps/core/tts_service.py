...
class MultiTTSService:
    ...
    def _load_silero_model(self, config: dict):
        ...
        try:
            print("📥 Загружаем модель Silero TTS...")
    ...
    def _load_model(self):
        ...
        if not success:
            print(
                "❌ Не удалось загрузить основную модель, пробуем Silero как fallback..."
            )
            ...
            for ending in sentence_endings:
                if ending in buffer:
                    ...
                    for sentence in sentences[:-1]:
                        ...
                        if (
                            sentence and len(sentence) > 10
                        ):  # Минимальная длина для озвучки
                            try:
                                audio_b64 = self.text_to_audio_base64(sentence)
                                yield audio_b64
                            except Exception as e:
                                print(
                                    f"❌ Ошибка генерации аудио для: {sentence[:50]}... - {e}"
                                )
